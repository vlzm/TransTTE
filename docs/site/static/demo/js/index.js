...
// API base URL (task D1). On GitHub Pages, config.js sets window.API_BASE to the
// Azure Container Apps FQDN (task E). When this file is served by the backend
// itself (e.g. local `docker run`), window.API_BASE is unset, so we fall back to
// the current origin — a same-origin request, not subject to CORS.
const API_BASE = (window.API_BASE || `${location.protocol}//${location.host}`).replace(/\/+$/, "");

// ---- cold-start warmup (task D2) -------------------------------------------
// ACA scales the API to zero when idle. Poll /health until the container is warm
// before letting the user build a route, otherwise the first /get_path hits a
// cold container and times out. A full-screen overlay blocks interaction until
// then. Since all heavy assets load at module import (before uvicorn accepts
// requests), any 200 from /health means the container is fully warm.
window.__apiWarm = false;

function showSpinner(text) {
    const el = document.getElementById("warmup-overlay");
    if (!el) return;
    const msg = el.querySelector(".warmup-text");
...
async function warmup() {
    ...
    while (true) {
        try {
            ...
            clearTimeout(t);
            if (r.ok) break;
        } catch (_) { /* container still asleep / starting up — retry */ }
        await new Promise(res => setTimeout(res, 2000));
    }
    window.__apiWarm = true;
    hideSpinner();
}
...
// alert(unputs_value[0])
        ...
        if (counter==2){
            console.log("kk");
            // Guard: ignore route requests until the API is warm (task D2). The
            // overlay normally blocks clicks, but this is defence in depth.
            if (!window.__apiWarm) {
                counter = 0;
                coords = [[0,0],[0,0]];
                return;
